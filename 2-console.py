        ...
        if grp_attr[0] not in ["\"", "{", "\'"]:
            ...
            if i_start < 0:
                ...
                if i_start < 0:
                    ...
                    try:
                        if int(grp_val):
                               setattr(obj, attr_name, int(grp_val))
                               obj.save()
                               return
                    except:
                        try:
                            if float(grp_val):
                               setattr(obj, attr_name, float(grp_val))
                               obj.save()
                               return
                        except:
                               setattr(obj, attr_name, grp_val)
                               obj.save()
                               return
                else:
                    i_end = grp_attr.find("\'", i_start + 1)
                    attr_val = grp_attr[i_start + 1:i_end]
