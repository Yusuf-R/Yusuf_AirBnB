...
class HBNBCommand(cmd.Cmd):
    ...
    def default(self, line):
        """This method called on an input line when the command prefix is not recognized"""
    ...
    def input_refactored(self, line):
        ...
        if len(grp_args) > 0 and grp_console_cmd in HBNBCommand.spec2_cmds:
            ...
            syntax_refactored = grp_console_cmd + " " + grp_class + " " + id_syntax
            self.onecmd(syntax_refactored)
            return syntax_refactored
        if len(grp_args) > 0 and grp_console_cmd == "update":
            ...
            if update_syntax[0] in ["\"", "\'"]:
                ...
                attr_val = grp.group(2).strip()
                attr_val = '"' + attr_val + '"'
                syntax_refactored = grp_console_cmd + " " + grp_class + " " + id_syntax + " " + attr_name + " " + attr_val
            ...
            if update_syntax[0] == "{":
                ...
                syntax_refactored = grp_console_cmd + " " + grp_class + " " + id_syntax + " " + dict_syntax
                self.onecmd(syntax_refactored)
                return syntax_refactored
            else:
                return line
